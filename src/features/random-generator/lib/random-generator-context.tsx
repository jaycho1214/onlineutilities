...
type RandomGeneratorAction =
  ...
  | {
      type: "UPDATE_CONFIG";
      payload: { type: GeneratorType; config: GeneratorConfig };
    }
...
const RandomGeneratorContext = createContext<
  RandomGeneratorContextInterface | undefined
>(undefined);
...
export function RandomGeneratorProvider({
  children,
}: {
  ...
  const generateValues = useCallback(
    async (type: GeneratorType, config: GeneratorConfig): Promise<string[]> => {
      ...
      try {
        ...
        // Generate values
        const results = await (
          generator as (config: GeneratorConfig) => Promise<string[]>
        )(config);
    ...
    async (id: string) => {
      try {
        ...
        if (success) {
          dispatch({
            type: "SET_HISTORY",
            payload: state.history.filter((entry) => entry.id !== id),
          });
        }
      } catch (error) {
  ...
  const savePreset = useCallback(
    async (name: string, type: GeneratorType, config: GeneratorConfig) => {
      try {
  ...
  const updatePreset = useCallback(
    async (id: string, updates: Partial<GeneratorPreset>) => {
      try {
  ...
  const loadPreset = useCallback(
    (preset: GeneratorPreset) => {
      ...
      dispatch({
        ...
        payload: {
          type: preset.type,
          config: preset.config as unknown as GeneratorConfig,
        },
      });
      toast.success(t("notifications.presetLoaded"));
    },
