...
export function CuidControls({ config, onChange }: CuidControlsProps) {
  ...
  const handleChange = <K extends keyof CuidConfig>(
    field: K,
    value: CuidConfig[K],
  ) => {
  ...
  const calculateConflictProbability = () => {
    ...
    return scenarios.map((scenario) => {
      if (scenario.users >= totalPossibleCuids)
        return { ...scenario, probability: 100 };

      // Birthday paradox: probability of at least one collision
      const probability =
        1 - Math.exp(-Math.pow(scenario.users, 2) / (2 * totalPossibleCuids));
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <Select
          ...
          onValueChange={(value: "cuid" | "cuid2") =>
