...
export function NanoidControls({ config, onChange }: NanoidControlsProps) {
  ...
  const handleChange = <K extends keyof NanoidConfig>(field: K, value: NanoidConfig[K]) => {
  ...
  const getAlphabetSize = (preset: NanoidPreset, customAlphabet: string) => {
    ...
    switch (preset) {
      case "default": return 64; // _-0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ
      case "alphanumeric": return 62; // 0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ
      case "numbers": return 10; // 0123456789
      case "lowercase": return 26; // abcdefghijklmnopqrstuvwxyz
      case "uppercase": return 26; // ABCDEFGHIJKLMNOPQRSTUVWXYZ
      default: return 64;
    }
  };
  ...
  const calculateConflictProbability = () => {
    ...
    return scenarios.map(scenario => {
      if (scenario.users >= totalPossibleIds) return { ...scenario, probability: 100 };
      
      // Birthday paradox: probability of at least one collision
      const probability = 1 - Math.exp(-Math.pow(scenario.users, 2) / (2 * totalPossibleIds));
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        ...
        <Input
          ...
          max={1000}
          value={config.length}
          onChange={(e) => handleChange("length", parseInt(e.target.value) || 21)}
          className="w-20"
        />
      </div>
      ...
      <div className="md:col-span-2">
        ...
        >
          ...
          <SelectContent>
            ...
            <SelectItem value="alphanumeric">{t("presets.alphanumeric")}</SelectItem>
        ...
        <div className="p-3 bg-blue-500/10 dark:bg-blue-400/5 rounded-lg border border-blue-500/20 dark:border-blue-400/20 backdrop-blur-sm">
          ...
          <div className="mt-2 pt-2 border-t border-blue-200 dark:border-blue-700">
            <p className="text-xs text-blue-600 dark:text-blue-400">
              💡 For production: Consider using the <code className="font-mono bg-blue-100 dark:bg-blue-900 px-1 rounded">nanoid</code> npm package
            </p>
          </div>
        </div>
      </div>
