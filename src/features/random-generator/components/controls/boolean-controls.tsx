...
export function BooleanControls({ config, onChange }: BooleanControlsProps) {
  ...
  const handleChange = <K extends keyof BooleanConfig>(field: K, value: BooleanConfig[K]) => {
  ...
  const calculateConflictProbability = () => {
    ...
    return scenarios.map(scenario => {
      if (scenario.users >= totalPossibleValues) return { ...scenario, probability: 100 };
      
      // Birthday paradox: probability of at least one collision
      const probability = 1 - Math.exp(-Math.pow(scenario.users, 2) / (2 * totalPossibleValues));
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <Select 
          value={config.format} 
          onValueChange={(value: BooleanFormat) => handleChange("format", value)}
        >
          <SelectTrigger>
            <SelectValue />
          </SelectTrigger>
      ...
      <div>
        <div className="p-3 bg-blue-500/10 dark:bg-blue-400/5 rounded-lg border border-blue-500/20 dark:border-blue-400/20 backdrop-blur-sm">
          ...
          <p className="text-xs text-blue-700 dark:text-blue-300 mt-2">
            Boolean values have only 2 possible states (very high collision rate)
          </p>
        </div>
      </div>
      ...
      <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
        <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">Preview values:</p>
        <div className="flex gap-4 text-sm font-mono text-gray-900 dark:text-gray-100">
          <span>True: {config.format === "boolean" ? "true" : config.format === "binary" ? "1" : "Yes"}</span>
          <span>False: {config.format === "boolean" ? "false" : config.format === "binary" ? "0" : "No"}</span>
        </div>
      </div>
    </div>
  );
