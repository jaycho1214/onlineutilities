...
function FileLoadingIndicatorComponent({
  fileLoading,
  className,
}: FileLoadingIndicatorProps) {
  ...
  return (
    ...
    >
      ...
      {/* Detection Results */}
      {fileLoading.detectedFormat &&
        !fileLoading.isLoading &&
        !fileLoading.isDetecting && (
          <div className="flex items-center gap-2 text-sm">
            <Zap className="h-4 w-4 text-blue-500" />
            <span className="text-foreground/70">
              {t("dragDrop.fileInfo.detected", {
                format: fileLoading.detectedFormat.toUpperCase(),
              })}
            </span>
      ...
      {fileLoading.detectionConfidence &&
        ...
        !fileLoading.isDetecting && (
          <div className="flex items-center gap-2 text-xs text-yellow-600 bg-yellow-50 dark:bg-yellow-900/20 p-2 rounded border border-yellow-200 dark:border-yellow-800">
            ...
            <span>
              {t("notifications.lowConfidenceDetection", {
                confidence: fileLoading.detectionConfidence,
              })}
            </span>
          </div>
        )}
