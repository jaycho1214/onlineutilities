...
export function validateFileSize(
  ...
  const formatKey =
    formatterType?.toUpperCase() as keyof typeof FILE_SIZE_LIMITS;
...
export function validateContentSize(
  ...
  const formatKey =
    formatterType.toUpperCase() as keyof typeof CONTENT_SIZE_LIMITS;
...
export function parseJsonError(error: Error, content: string): FormatterError {
  ...
  return {
    ...
    details: position
      ? `Error occurred at character position ${position}`
      : undefined,
    line,
    column,
    position,
    ...
    if (error instanceof Error) {
      switch (formatterType) {
        case "json":
          ...
          break;
        case "xml":
          formatterError = parseXmlError(error);
          break;
        case "csv":
          formatterError = parseCsvError(error);
          break;
        default:
          formatterError = {
