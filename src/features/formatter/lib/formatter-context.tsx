...
const FormatterContext = createContext<FormatterContextValue | undefined>(
  undefined,
);
...
export function FormatterProvider({ children }: FormatterProviderProps) {
  ...
  const detectContentType = useCallback(
    (content: string, filename?: string): FormatterType => {
      ...
      if (
        trimmed.startsWith("<") &&
        (trimmed.includes("<?xml") || trimmed.includes("</"))
      ) {
        return "xml";
      }
    ...
    (input: string) => {
      ...
      if (input.trim().length > 50) {
        ...
        if (detectedType !== state.type) {
          ...
          toast.info(
            `Content appears to be ${detectedType.toUpperCase()}. Switch format?`,
            {
              action: {
                label: "Switch",
  ...
  const formatInput = useCallback(async () => {
    if (!state.input.trim()) {
      toast.error(
        t("notifications.emptyInput", { type: state.type.toUpperCase() }),
      );
      return;
    }
    ...
    try {
      ...
      if (result.success && result.output) {
        ...
        toast.error(
          result.error ||
            t("notifications.error", { type: state.type.toUpperCase() }),
        );
      }
    } catch {
  ...
  const handleDrop: React.DragEventHandler<HTMLDivElement> = useCallback(
    async (e) => {
      ...
      if (!hasValidExtension) {
        toast.error(
          "Unsupported file type. Please use .json, .csv, .xml, or .txt files.",
        );
        return;
      }
      ...
      try {
        ...
        reader.onload = (e) => {
          ...
          if (content) {
            ...
            toast.success(
              `File loaded and format detected: ${detectedType.toUpperCase()}`,
            );
          } else {
            setState((prev) => ({
              ...prev,
  ...
  const handleFileLoadInternal = useCallback(
    async (file: File) => {
        ...
        if (result.success && result.content) {
          ...
          if (result.detectedFormat && result.detectedFormat !== state.type) {
            ...
            if (confidence >= 70) {
              ...
              toast.success(
                t("notifications.formatDetected", {
                  type: result.detectedFormat!.toUpperCase(),
                }),
              );
              ...
              toast.info(
                t("notifications.formatDetectionConfirm", {
                  type: result.detectedFormat!.toUpperCase(),
                }),
              );
            } else {
  ...
  const handleFileLoadAPI = useCallback(
    async (file: File) => {
