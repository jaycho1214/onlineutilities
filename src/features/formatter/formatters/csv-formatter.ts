...
function detectCsvDelimiter(content: string): {
  ...
  const results = delimiters.map(({ type, char }) => {
    ...
    const lineCounts = lines
      .map((line) => (line.match(new RegExp(`\\${char}`, "g")) || []).length)
      .filter((count) => count > 0);

    const avgCount =
      lineCounts.length > 0
        ? lineCounts.reduce((sum, c) => sum + c, 0) / lineCounts.length
        : 0;

    const variance =
      lineCounts.length > 0
        ? lineCounts.reduce((sum, c) => sum + Math.pow(c - avgCount, 2), 0) /
          lineCounts.length
        : Infinity;
  ...
  const confidence =
    best.score > 0
      ? Math.min(1, best.score / (results[1]?.score || best.score + 1))
      : 0;
...
export class CsvFormatter extends BaseFormatter {
  ...
  detectContent(content: string): ContentDetectionResult {
    ...
    const columnCounts = lines
      .slice(0, 10)
      .map((line) => parseCsvLine(line, delimiterChar).length);
