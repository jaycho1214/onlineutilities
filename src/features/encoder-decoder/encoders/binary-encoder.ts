...
export class BinaryEncoder extends AbstractEncoder {
    ...
    try {
      ...
      for (const binaryGroup of binaryGroups) {
        ...
        if (!/^[01]+$/.test(binaryGroup)) {
          return this.createResult(false, undefined, `Invalid binary: ${binaryGroup}`);
        }
        
        // Convert binary to decimal
        const decimal = parseInt(binaryGroup, 2);
        
        if (decimal > 1114111) { // Unicode range
          return this.createResult(false, undefined, `Binary value out of range: ${binaryGroup}`);
        }
  ...
  detect(input: string): number {
    ...
    const eightBitGroups = groups.filter(group => group.length === 8 && /^[01]{8}$/.test(group));
